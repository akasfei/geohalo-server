...
function BufferPacker () {
  ...
  this.unpack = function (buffer, callback) {
    ...
    // min packet length is 3
    if(buffer.length < 3) {
      var error = new Error('packet length less than 3');
      callback(error, null);
    }
    ...
    switch (type) {
      case this.C2S_PackType_Heartbeat:
        checkJsonFormatAndField(buffer, [], callback);
        break;
...
function checkJsonFormatAndField(data, fieldArray, callback){
  ...
  var type  = data.readUInt8(2);
  var len   = data.readUInt16BE(0);
  console.log('unpack type ' + type);
  try{
  ...
  callback(null, {type: type, obj: obj});
}
