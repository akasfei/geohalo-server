...
SocketEvent.prototype.checkSession = function (email, sessionid, callback) {
  var _this = this;
  var _db = _this.db;
  _db.client.connect(_db.mongourl, function (err, db){
    if(err){
    ...
    db.collection('sessions', function (err, collection){
      ...
      collection.find({_id:sessionid}).toArray(function (err, items){
        if(err){
          return logger.error('[ SocketEvent ]', '[ checkSession ]', err);
        }
        ...
        if(items.length == 1){
          try{
            var session = JSON.parse(items[0].session);
        ...
        // invalid sessionid.
        if(items.length < 1){
          return logger.fatal('[ SocketEvent ]', '[ checkSession ]', 
              'invalid sessionid');
        }
        // fatal
        if(items.length > 1){
          return logger.fatal('[ SocketEvent ]', '[ checkSession ]', 
              'use one sessionid to query mongodb, and it return' +  
              'more than one object.');
          callback(false);
        }

        logger.fatal('[ SocketEvent ]', '[ checkSession ]', "it's impossible!");
        callback(false);
      });
    });
  });
